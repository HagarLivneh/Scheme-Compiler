...
%macro MAKE_LITERAL_VOID 0
		MAKE_LITERAL_WORD T_VOID
%endmacro

%macro MAKE_LITERAL_NIL 0
		MAKE_LITERAL_WORD T_NIL
%endmacro
...
%macro SHIFT_FRAME 1 ; %1 = size of frame (constant)
	mov r13, rbp
	mov r11, rax ; r11 = backup for rax
	mov r15, %1
	add r15, 4 ; r15 holds the new frame size (without mul with 8)
	mov r14, PARAM_COUNT
	mov rax, PARAM_COUNT
	add rax, 4 ; r14 holds the old frame size (without mul with 8)
	push rbx
	%assign i 1
	%rep %1
		dec rax
		mov rbx, qword [rbp-WORD_SIZE*i] ; rbx holds the new arg
		mov qword [rbp+WORD_SIZE*rax], rbx ; replace the old arg with new
	%assign i i+1
	%endrep
	pop rbx
	dec rax
	mov qword [rbp+WORD_SIZE*rax], %1;num of args of new
	dec rax
	mov qword [rbp+WORD_SIZE*rax], r10;new env
	dec rax
	mov qword [rbp+WORD_SIZE*rax], r9;old ret address
	dec rax
	mov qword [rbp+WORD_SIZE*rax], r8;old rbp

	add r14, 5
	shl r14, 3
	add rsp, r14
	
	mov rax, r11			; rax <- old rax value
